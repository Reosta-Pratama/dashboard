...
import { BorderTh } from '@/components/table/border/components/th';
import { BorderTd } from '@/components/table/border/components/td';

interface CodeHoverTableProps {
    active: boolean;
}

export const CodeHoverTable: React.FC<CodeHoverTableProps> = ({active}) => {
  return (
    <div className="p-5">
        {active == false 
            ? 
                ...
                <tbody>
                    {
                      DummyTable.map((user, index) => (
                        <tr key={index + 1}
                            className='duration-200 ease-in-out hover:bg-slate-100'>
...
const tableAsString = tables
    .map((item) => {
        return `<table class="w-full text-left ${item.tableClass}"> 
          ...
          <tbody>
          ${DummyTable.map((user, index) => (
            `<tr key=${index + 1} class="duration-200 ease-in-out hover:bg-slate-100">
                                    ...
                                    <br />{``}  &lt;<span className='text-[#22863a]'>tbody</span>
                                      {
                                          DummyTable.map((user, index) => {
                                            return (
                                              <>
                                                <br />{` `}  &lt;<span className='text-[#22863a]'>tr</span>{` `}<span className='text-[#005cc5]'>class</span>{`="duration-200 ease-in-out hover:bg-slate-100"`}&gt;
