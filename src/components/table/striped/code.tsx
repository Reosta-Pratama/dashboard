...
import { BasicTd } from '@/components/table/basic/components/td';
import { BasicTh } from '@/components/table/basic/components/th';
import { DummyTable } from '@/dummys/dummy-table';
import React, { useState } from 'react'
import { FiFile } from 'react-icons/fi'
import { BoxCode } from '../../show-code/box';

interface CodeStripedTableProps {
    active: boolean;
}

export const CodeStripedTable: React.FC<CodeStripedTableProps> = ({active}) => {
  return (
    <div className="p-5">
        {active == false 
            ? 
...
const tableAsString = tables
    .map((item) => {
        return `<table class="w-full text-left ${item.tableClass}"> 
          ...
          <tbody>
          ${DummyTable.map((user, index) => (
            `<tr key=${index + 1} class="odd:bg-slate-100">
